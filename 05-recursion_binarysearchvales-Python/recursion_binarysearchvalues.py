...
def recursion_binarysearchvalues(L, v,nL=None, l=0, high=0):
	if nL == None:
		nL = []
		high = len(L)-1
	if L[l:high+1] == []:
		...
		midval = L[mid]
		if v == midval:
			nL.append((L.index(v), v))
			return nL
		elif midval < v:
			nL.append((L.index(midval), midval))
			l = mid+1
			return recursion_binarysearchvalues(L, v, nL, l, high)
		elif midval > v:
			nL.append((L.index(midval), midval))
			high = mid-1
			return recursion_binarysearchvalues(L, v, nL, l, high)
